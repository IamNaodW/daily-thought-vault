...
function MainEditor({ onSave, moodData }) {
  const [entry, setEntry] = useState("");
  const [mood, setMood] = useState(2);
  const [loading, setLoading] = useState(false);

  const handleSave = async () => {
    if (!entry.trim()) return; // ignore empty save

    setLoading(true);
    try {
      // POST new thought to backend
      const res = await axios.post("http://localhost:5000/api/thoughts", {
        text: entry,
        mood,
      });
      ...
      // Reset editor
      setEntry("");
      setMood(2); // optional: reset mood to neutral
    } catch (err) {
      console.error("Failed to save thought:", err);
      alert("Failed to save thought. Check console for details.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="flex-1 flex flex-col max-w-5xl mx-auto w-full px-8 md:px-16 py-12">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
          ...
          <p className="text-zinc-500 mt-1 font-medium">Capture your silence.</p>
        </div>
            ...
            return (
              <button
                ...
                className={`relative p-2 w-12 h-12 rounded-xl text-2xl transition-all duration-300
                  ${isActive ? `${m.bg} ${m.color} ${m.border} scale-110 shadow-lg border` : "opacity-30 hover:opacity-100 grayscale hover:grayscale-0"}
                `}
                title={m.label}
                disabled={loading}
              >
      ...
      <footer className="mt-8 flex justify-end items-center gap-6">
        ...
        <button
          onClick={handleSave}
          className={`px-8 py-3 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-500 transition-all active:scale-95 ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
          disabled={loading}
        >
